#ifndef __PID_UPPER_COMPUTER_H__
#define __PID_UPPER_COMPUTER_H__

#include "headfile.h"
/*
这个文件用于野火调试助手PID上位机与下位机实现通信

上位机->下位机的函数
receiving_process、protocol_data_recv、protocol_init、
其中函数protocol_init放在while(1)之前，receiving_process放在while(1)中，protocol_data_recv放在串口接收回调函数中每次接收1字节
其中函数内部具体的实现函数需要自己定义，有：
set_pid_paramter_cmd、set_pid_actual_val_cmd、pid_start_cmd、
pid_stop_cmd、pid_reset_cmd、set_pid_period_cmd
